...
public class Plant extends TagClass {
  ...
  public void secondTick(TagHandler tagHandler, Entity entity) {
    ...
    if (entity.containsCustomData("totalPlantGrowTime")
      ...
      Builder entityBuilder =
          entity.toBuilder()
              ...
              .setSize(Vector.newBuilder().setX(updatedSizeX).setY(updatedSizeY).build());
    ...
    // check to see if its grown
      ...
      int randomSproutChance =
          DataUtil.getInt(entity, "randomSproutChance", defaultRandomSproutChance);
      int totalPlantGrowTime =
          DataUtil.getInt(entity, "totalPlantGrowTime", defaultTotalPlantGrowTime);
      ...
      if (Math.round(Math.random() * randomSproutChance) == 1) {
        ...
        ArrayList<Entity> foundEntities =
            tagHandler
                ...
                .getAllEntitiesInRadius(
                    spawnVector,
                    entity.getMap(),
                    Math.hypot(entity.getSize().getX(), entity.getSize().getY()));

        if (foundEntities.size() > 0) return;

        Tile originalTile =
            tagHandler.getPlaneManager().getTileAt(entity.getPosition(), entity.getMap());
        Tile newTile = tagHandler.getPlaneManager().getTileAt(spawnVector, entity.getMap());

        if (newTile == null
            || originalTile == null
            || !newTile.getName().equals(originalTile.getName())) return;
        ...
        if (sprout.getTagsList().contains("harvestable")) {
          sprout = sprout.toBuilder().setDisplayTexture("empty " + sprout.getName()).build();
        }
