...
public class Locator extends TagClass {
  ...
  public Entity interact(TagHandler tagHandler, Entity interactingEntity, Entity thisEntity) {
    ...
    ArrayList<Entity> entities =
        tagHandler.getEntityFinder().getAllEntitiesInRadis(interactingEntity, 250);
    int closestIndex = 0;
    ...
    for (int i = 0; i < entities.size(); i++) {
      ...
      if (validTarget(entity)) {
        double distanceSquared =
            VectorMath.distanceSquared(entity.getPosition(), thisEntity.getPosition());
    ...
    if (closestDistanceSquared != 0) {
      ...
      tagHandler
          .getChatManager()
          .messageAllPlayers(
              "[Locator] target located at x:"
                  + closestEntity.getPosition().getX()
                  + ", y:"
                  + closestEntity.getPosition().getY());
