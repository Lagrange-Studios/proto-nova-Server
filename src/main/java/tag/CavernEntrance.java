...
public class CavernEntrance extends TagClass {
  ...
  public void secondTick(TagHandler tagHandler, Entity entity) {
	  ...
	  if (!entity.containsCustomData("exitId")) {
		  CelestialObject object = tagHandler.getCelestialObjectManager().getCelestialObjectFromEntity(entity);
		  
		  int undergroundPlaneId = object.getUndergroundPlaneId();
		  
		  Entity newExit = tagHandler.getAssetManager().getEntity("cavern exit", undergroundPlaneId, entity.getPosition());
		  newExit = newExit.toBuilder()
				  .putCustomData("entranceId", DataUtil.newInt(entity.getId()))
				  .build();
		  
		  tagHandler.updateEntity(newExit);
		  
		  entity = entity.toBuilder()
				  .putCustomData("exitId", DataUtil.newInt(newExit.getId()))
				  .build();
  ...
  public Entity interact(TagHandler tagHandler, Entity interactingEntity, Entity thisEntity) {
	  ...
	  Entity exit = tagHandler.getEntityManager().getEntity(thisEntity.getCustomDataMap().get("exitId").getIntValue());
