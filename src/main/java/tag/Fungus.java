...
public class Fungus extends TagClass {
  ...
  // tile info
  public static final HashSet<String> allowedTiles =
      new HashSet<>(Arrays.asList("grass", "stone", "sand"));
  public static final HashSet<String> conversionTiles = new HashSet<>(Arrays.asList("grass"));

  // fungus names
  public static final HashSet<String> fungusNames =
      new HashSet<>(Arrays.asList("fungus vein", "fungus spore", "fortifeid fungus vein"));
  ...
  public static Tile convertTile(Tile tile) {
    if (Fungus.conversionTiles.contains(tile.getName())) {
      tile = tile.toBuilder().setName("dirt").build();
    }
    return tile;
  }
  ...
  public void secondTick(TagHandler tagHandler, Entity entity) {
    ...
    Entity parentSpore =
        tagHandler.getEntityManager().getEntity(DataUtil.getInt(entity, "parentSpore", 0));
    if (parentSpore != null && parentSpore.getName().equals("fungus spore")) {
      ...
      if (entity.getName().equals("fungus spore")) {
        ArrayList<Entity> closeEntities =
            tagHandler.getEntityFinder().getAllEntitiesInRadis(entity, SEARCH_RANGE);
        ...
        for (Entity target : closeEntities) {
          if (target.getName().equals("human")) {
            double distanceSquared =
                VectorMath.distance(entity.getPosition(), target.getPosition());
        ...
        if (closestTarget != null) {
          ...
          entity = entity.toBuilder().removeCustomData("target").build();
      ...
      if (Random.randomInt(1, chanceOfGrowthPerSecond) == 1
        ...
        if (tile != null && allowedTiles.contains(tile.getName())) {
          ...
          for (Entity foundEntity :
              tagHandler
                  .getEntityFinder()
                  .getAllEntitiesInRadius(newPosition, entity.getMap(), .99)) {
          ...
          if (open) {
            Entity newClone =
                tagHandler.getAssetManager().getEntity("fungus vein", entity.getMap());
      ...
      // chance to fortify
      else if (entity.getName().equals("fungus vein")
          && Random.randomInt(1, evolveChancePerSecond) == 1
        ...
        if (Random.randomInt(1, fungusMonsterChance) == 1
            && monsterCount < MAX_MONSTERS_PER_SPORE) {
          Entity newMonster =
              tagHandler.getAssetManager().getEntity("fungus monster", entity.getMap());

          newMonster =
              newMonster.toBuilder()
          ...
          // System.out.println("[Fungus] new monster at:
          // "+newMonster.getPosition().getX()+","+newMonster.getPosition().getY());
          parentSpore =
              parentSpore.toBuilder()
                  .putCustomData("monsterCount", DataUtil.newInt(monsterCount + 1))
                  .build();

          tagHandler.updateEntity(parentSpore);

          // System.out.println("[Fungus] new monster count for parent spore at x:
          // "+parentSpore.getPosition().getX()+" y: "+parentSpore.getPosition().getY() + " monster
          // count: "+ parentSpore.getInventorySlotsMap().get("monsterCount"));
        }
        entity = entity.toBuilder().setName("fortifeid fungus vein").build();
