...
public class Cataclysm extends GamemodeClass {
   ...
   *
   * <p>state1: peace, the cataclysm does not act and the players have time to build state2: intro,
   * the cataclysm begins to affect the world in noticeable ways state3: end, the cataclysm becomes
   * full force and the effects are felt across the world
   */
  public Cataclysm(
      Console console,
  ...
  private void loadAllloadCataclysms() {
    try (ScanResult scanResult = new ClassGraph().acceptPackages("cataclysm").scan()) {
      for (ClassInfo classInfo : scanResult.getAllClasses()) {
        ...
        try {
          ...
          nameToStaticCataclysm.put(
              (String) staticClass.getMethod("getName").invoke(staticClass), staticClass);
