...
public class Server {
  ...
  private long saveInterval =
      15 * 60 * 1000; // 15 minutes in milliseconds (real time, not TPS-dependent)
  ...
  public Server(boolean headless) {
    ...
    assetManager =
        new AssetManager(
            entityManager, serverLoader.loadEntityAssets(), console, serverLoader.loadTypes());
    ...
    soundFinder =
        new SoundFinder(entityManager.getAllEntities(), soundManager.getAllSounds(), chunkManager);
    chatFinder =
        new ChatFinder(entityManager.getAllEntities(), chatManager.getAllChats(), chunkManager);
    ...
    craftingManager =
        new CraftingManager(
            entityManager, serverLoader.loadCraftingRecipes(), console, assetManager, planeManager);
    ...
    if (shouldGenerate) {
      ...
      for (Entity entity :
          entityFinder.getAllEntitiesInRadius(Vector.newBuilder().setX(0).setY(0).build(), 1, 2)) {
      ...
      Entity lightEntity =
          assetManager.getEntity("clear", 1, Vector.newBuilder().setX(0).setY(0).build());
    ...
    actionHandler =
        new ActionHandler(
            ...
            consumptionManager);
    serverSaver =
        new ServerSaver(this, entityManager, planeManager, celestialObjectManager, gamemodeManager);
    ...
    serverReady = true;
    console.print(
        "Server ready. Game port: "
            + ServerConfig.getInstance().getGameSocketPort()
            + ", HTTPS status/client port: "
            + ServerConfig.getInstance().getStatusHttpPort());
  ...
  private void startThread() {
    try {
      ...
      java.lang.management.OperatingSystemMXBean osBeamBase =
      ...
      ramLimit =
          (long) ServerConfig.getInstance().getRamLimit() * 1024 * 1024; // Convert MB to bytes
      workerThreadLimit = ServerConfig.getInstance().getWorkerThreadLimit();

      scheduler =
          Executors.newScheduledThreadPool(
              3,
              ResourceDiagnostics.threadFactory(
                  "Server-Scheduler")); // tick, idle check, resource check
  ...
  private void tick() throws Exception {
    // Create a copy to avoid ConcurrentModificationException when players disconnect during
    // iteration
  ...
  private void checkResourceLimits() {
    ...
    if (processorLimit > 0) {
      ...
      if (processorLoad > limitedLoad) {
        console.print(
            "⚠ Processor usage exceeds limit: "
                + String.format("%.2f%%", processorLoad * 100)
                + " (Limit: "
                + effectiveLimit
                + "/"
                + availableProcessors
    ...
    if (workerThreadLimit > 0) {
      ...
      if (threadCount > workerThreadLimit) {
        console.print(
            "⚠ Thread count exceeds limit: " + threadCount + " (Limit: " + workerThreadLimit + ")");
      }
    }
  }
