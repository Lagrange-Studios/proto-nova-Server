...
public class Main {
  ...
  private static void initializeOrCheckConfig(boolean initializeOnly) {
    try {
      ...
      System.out.println(
          (initializeOnly ? "Server configuration is ready: " : "Server configuration is valid: ")
              + ServerConfig.getConfigPath().toAbsolutePath());
      System.out.println(
          "Base game port: "
              + ServerConfig.getInstance().getGameSocketPort()
              + "; companion HTTPS/client port: "
      ...
      try (java.net.Socket socket = new java.net.Socket()) {
        socket.connect(
            new java.net.InetSocketAddress(
                "127.0.0.1", ServerConfig.getInstance().getGameSocketPort()),
            2_000);
      }
  ...
  private static void printHelp() {
    ...
    System.out.println(
        "  --headless, --nogui  Run with the terminal console (recommended for hosting)");
