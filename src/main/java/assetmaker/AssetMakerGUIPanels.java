...
/**
 * Builds the editor tabs. Each method below represents one logical asset section. To add a new UI
 * section, create a buildXTab method, add its controls to AssetMakerGUI, add the tab in
 * AssetMakerGUI.buildEditorTabs() using {@code wrapEditorTab(...)} so it remains reachable on small
 * windows, and update AssetMakerGUIController for loading and saving.
 */
class AssetMakerGUIPanels {
  ...
  JPanel buildIdentityTab() {
    ...
    JLabel startHelp =
        new JLabel("Choose the closest kind of thing first. You can change any answer afterward.");
    ...
    row++;
    c.weightx = 0;
    addLabel(p, c, "What is it called?", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.nameField, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Saved ID (automatic):", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.idField, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Starting map number:", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.mapField, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Which way does it face?", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.directionCombo, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Starting hand slot:", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.selectedSlotField, c);
    row++;
    ...
    applyStarter.addActionListener(
        e -> gui.applyStarterPreset(String.valueOf(gui.starterTypeCombo.getSelectedItem())));
    return p;
  }
  ...
  JPanel buildMovementTab() {
    ...
    int row = 0;
    JLabel help =
        new JLabel("Tip: 1 means one map tile. Most saved assets can start at X = 0 and Y = 0.");
    ...
    row++;
    c.weightx = 0;
    addLabel(p, c, "Start left/right (X):", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.posXField, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Start up/down (Y):", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.posYField, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Moving left/right now:", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.velXField, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Moving up/down now:", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.velYField, c);
    row++;

    row++;
    c.weightx = 0;
    addLabel(p, c, "Width (tiles):", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.sizeXField, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Height (tiles):", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.sizeYField, c);
    row++;

    row++;
    c.weightx = 0;
    addLabel(p, c, "Speed right now:", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.speedField, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Fastest speed:", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.maxSpeedField, c);
    row++;
    c.weightx = 0;
    addLabel(p, c, "Touching reach (tiles):", 0, row);
    c.weightx = 1;
    c.gridx = 1;
    p.add(gui.reachField, c);
    row++;
  ...
  JPanel buildItemTab() {
    ...
    JLabel durabilityHelp =
        new JLabel(
            "<html>Items are indestructible unless <b>Can be damaged and destroyed</b> is enabled. "
    ...
    JLabel invHelp =
        new JLabel(
            "<html>Put one on each line. Example: <i>leftHand = #123</i>. "
  ...
  JPanel buildChemistryTab() {
    ...
    JLabel customHelp =
        new JLabel(
            "Each key stores all five protobuf value types; unused values can stay at their defaults.");
    ...
    add.addActionListener(
        e -> gui.customDataModel.addRow(new Object[] {"newKey", 0, 0.0f, 0.0d, false, ""}));
  ...
  JPanel buildTagsTab() {
    ...
    JLabel tagsHelp =
        new JLabel(
            "<html>Behavior words tell the game what this thing can do. Separate them with commas. "
    ...
    c.gridy = 1;
    c.weightx = 0;
    addLabel(root, c, "Behavior words:", 0, 1);
    c.weightx = 1;
    c.gridx = 1;
    root.add(gui.tagsField, c);

    c.weightx = 0;
    addLabel(root, c, "Picture name:", 0, 2);
    c.weightx = 1;
    c.gridx = 1;
    root.add(gui.displayTextureField, c);

    c.weightx = 0;
    addLabel(root, c, "Optional color:", 0, 3);
    c.weightx = 1;
    c.gridx = 1;
    root.add(gui.hexColorField, c);

    c.weightx = 0;
    addLabel(root, c, "Render priority:", 0, 4);
    c.weightx = 1;
    c.gridx = 1;
    root.add(gui.renderPrioritySpinner, c);

    JLabel colorHelp =
        new JLabel(
            "<html>Optional tint color in <code>#RRGGBB</code> or <code>#AARRGGBB</code> "
  ...
  JPanel buildLootTableTab() {
    ...
    JLabel help =
        new JLabel(
            "<html>Define what items this entity drops when defeated. "
  ...
  JPanel buildPhysiologyTab() {
    ...
    JLabel help =
        new JLabel(
            "Easy choice: click 'Use a Normal Human Body' and leave the detailed numbers alone.");
    ...
    templates.setBorder(
        BorderFactory.createTitledBorder("Which saved organs should this body receive?"));
    ...
    JLabel detailHelp =
        new JLabel(
            "Detailed organ numbers are below. The normal-human button fills them in for you.");
  ...
  private JPanel buildStomachPanel() {
    ...
    addChemicalArea(
        panel, gui.stomachChemicalsField, "Contents (chemicalName=amountU, one per line):", 7);
    return panel;
  }
  ...
  private JPanel buildCirculationPanel() {
    ...
    addOrganField(
        panel,
        "Total fluid capacity (blood + chemicals, u):",
        gui.cardiovascularFluidCapacitySpinner,
        7);
    addChemicalArea(
        panel,
        gui.cardiovascularChemicalsField,
        "Bloodstream chemicals (chemicalName=amountU):",
        8);
    return panel;
  }

  private JPanel organPanel(
      String title, javax.swing.JCheckBox present, AssetMakerGUI.OrganStatusControls status) {
    JPanel panel = new JPanel(new GridBagLayout());
    ...
    c.gridx = 1;
    panel.add(status.healthPercent, c);
    c.gridx = 2;
    panel.add(new JLabel("Efficiency (%):"), c);
    c.gridx = 3;
    c.weightx = 1;
    panel.add(status.efficiencyPercent, c);
    c.weightx = 0;
    c.gridx = 0;
    c.gridy = 2;
    panel.add(new JLabel("Cybernetic power demand (u/s):"), c);
    c.gridx = 1;
    c.gridwidth = 3;
    c.weightx = 1;
    panel.add(status.powerUse, c);
    c.gridx = 0;
    c.gridy = 3;
    c.gridwidth = 1;
    c.weightx = 0;
    ...
    c.gridwidth = 3;
    c.weightx = 1;
    panel.add(status.nutritionUse, c);
    return panel;
  }
  ...
  private static void setBiological(
      AssetMakerGUI.OrganStatusControls status, double nutritionUsePerSecond) {
  ...
  JPanel buildAdvancedTab() {
    ...
    internal.add(
        new JLabel("Runtime/custom values. Leave blank unless another system expects them."),
        BorderLayout.NORTH);
