 ...
 *
 * <p>This class owns controls and layout only. Button behavior belongs in {@link
 * AssetMakerGUIController}; tab layout belongs in {@link AssetMakerGUIPanels}. To add a field, add
 * the control in the matching section below, place it on the matching tab, then read/write it in
 * the controller.
 */
public class AssetMakerGUI {
  ...
  final JComboBox<String> starterTypeCombo =
      new JComboBox<>(
          new String[] {
  ...
  final JComboBox<Direction> directionCombo =
      new JComboBox<>(
  ...
  final JSpinner renderPrioritySpinner =
  ...
  final JSpinner maxHealthSpinner =
      new JSpinner(new SpinnerNumberModel(100, 0, Integer.MAX_VALUE, 1));
  final JSpinner critHealthSpinner =
      new JSpinner(new SpinnerNumberModel(50, 0, Integer.MAX_VALUE, 1));
  ...
  final JSpinner temperatureSpinner =
      new JSpinner(new SpinnerNumberModel(0, Integer.MIN_VALUE, Integer.MAX_VALUE, 1));
  final JTextArea entityChemicalsField = new JTextArea();
  final DefaultTableModel customDataModel =
  ...
  final DefaultTableModel lootTableModel =
      new DefaultTableModel(new Object[] {"Item Name", "Probability (%)", "Amount"}, 0) {
        @Override
        public Class<?> getColumnClass(int column) {
          switch (column) {
            case 0:
  ...
  final JSpinner internalSpaceSpinner =
  ...
  final JSpinner lungsOxygenSpinner =
      new JSpinner(new SpinnerNumberModel(0, 0, Integer.MAX_VALUE, 1));
  ...
  final JSpinner liverDetoxificationSpinner =
      new JSpinner(new SpinnerNumberModel(0, 0, Integer.MAX_VALUE, 1));
  ...
  private void addSimpleHelp() {
    ...
    idField.setToolTipText(
        "The server gives each spawned thing its own ID. You normally leave this alone.");
    ...
    renderPrioritySpinner.setToolTipText(
        "Higher numbers draw on top of entities with lower numbers.");
    ...
    lightRangeField.setToolTipText(
        "Leave blank for no light, or enter how many tiles it lights up.");
    anchoredBox.setToolTipText("Turn this on if it should stay in one place.");
    canCollideBox.setToolTipText(
        "Turn this on if other things should bump into it instead of passing through.");
    ...
    temperatureSpinner.setToolTipText(
        "The entity's current temperature used by chemical reactions.");
  }
  ...
  private void installDirtyTracking() {
    ...
    JTextField[] textFields = {
      ...
      reachField,
      lightRangeField,
      heartOrganAssetField,
      lungsOrganAssetField,
      liverOrganAssetField,
      brainOrganAssetField,
      stomachOrganAssetField
    };
    for (JTextField field : textFields) field.getDocument().addDocumentListener(documentListener);
    JTextArea[] textAreas = {
      inventorySlotsField,
      internalValuesField,
      stomachChemicalsField,
      cardiovascularChemicalsField,
    ...
    JSpinner[] otherSpinners = {
      renderPrioritySpinner,
      hitCooldownSpinner,
      maxHealthSpinner,
      critHealthSpinner,
      amountSpinner,
      internalSpaceSpinner,
      temperatureSpinner,
      heartBloodSpinner,
      heartMaxBloodSpinner,
      heartCirculationSpinner,
      heartOxygenUseSpinner,
      lungsOxygenSpinner,
      lungsOxygenUseSpinner,
      liverDetoxificationSpinner,
      liverOxygenUseSpinner,
      brainOxygenUseSpinner,
      stomachCapacitySpinner,
      stomachAbsorptionSpinner,
      stomachOxygenUseSpinner,
      cardiovascularOxygenSpinner,
      cardiovascularMaxOxygenSpinner,
      cardiovascularPowerSpinner,
      cardiovascularMaxPowerSpinner,
      cardiovascularNutritionSpinner,
      cardiovascularMaxNutritionSpinner,
      cardiovascularFluidCapacitySpinner
    };
    for (JSpinner spinner : otherSpinners) spinners.add(spinner);
    OrganStatusControls[] statuses = {
      heartStatus, lungsStatus, liverStatus, brainStatus, stomachStatus
    };
  ...
  /**
   * Makes an editor tab vertically scrollable without allowing horizontal scrolling to hide field
   * labels. Add new tabs through this helper too.
   */
