...
public class OrganAssetMigration {
  ...
  public static void main(String[] args) {
    ...
    humanOrgans.setCardiovascularSystem(
        humanOrgans.getCardiovascularSystem().toBuilder()
    ...
    if (!organs.hasHeart() && human.hasOrganAssetSlots()) {
      ...
      if (!assets.saveEntity("human", human))
        throw new IllegalStateException("Could not save human asset");
      return;
    }
    ...
    migrate(
        assets,
        "human brain",
        OrganComponent.newBuilder()
            .setBrain(organs.hasBrain() ? organs.getBrain() : Brain.newBuilder().build())
            .build());
    migrate(
        assets,
        "human stomach",
        OrganComponent.newBuilder()
            .setStomach(
                organs.hasStomach()
                    ? organs.getStomach()
                    : Stomach.newBuilder()
                        .setChemicalCapacity(50)
                        .setAbsorptionPerSecond(1)
                        .setOxygenUsePerSecond(0.5f)
                        .build())
            .build());

    Organs.Builder bodyState =
        organs.toBuilder().clearHeart().clearLungs().clearLiver().clearBrain().clearStomach();
    Entity migratedHuman =
        human.toBuilder()
            .setOrgans(bodyState)
            .setOrganAssetSlots(
                OrganAssetSlots.newBuilder()
            ...
            .build();
    if (!assets.saveEntity("human", migratedHuman))
      throw new IllegalStateException("Could not save human asset");
  }
  ...
  private static void migrate(AssetMaker assets, String name, OrganComponent component) {
    ...
    Entity.Builder builder =
        organ.toBuilder()
            ...
            .clearOrgans();
    if (!assets.saveEntity(name, builder.build()))
