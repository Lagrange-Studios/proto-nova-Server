...
public final class PhysiologySystem {
  ...
  public Entity update(Entity body, EntityManager entityManager, float breathableOxygen) {
    ...
    Entity bodyWithInstalledOrgans = body.toBuilder().setOrgans(installedOrgans).build();
    ...
    return updatedBody.toBuilder().setOrgans(bodyPhysiologyState).build();
  }
  ...
  private Entity updateBleeding(Entity entity) {
    ...
    damage.setBleedingPerSecond(Math.max(0, bleedingPerSecond - BLEEDING_RECOVERY_PER_SECOND));
    ...
    if (bleedingPerSecond > 0 && organs.hasHeart()) {
      ...
      organs.setHeart(heart.toBuilder().setBlood(Math.max(0, currentBlood - bleedingPerSecond)));
    }

    return entity.toBuilder().setDamage(damage).setOrgans(organs).build();
  }
  ...
  private void updateLungs(PhysiologyState state) {
    ...
    float oxygenUse =
        valueOrDefault(
            lungs.hasOxygenUsePerSecond(), lungs.getOxygenUsePerSecond(), DEFAULT_LUNG_OXYGEN_USE);
  ...
  private void updateHeart(PhysiologyState state) {
    ...
    float circulationPerSecond =
        valueOrDefault(
            heart.hasCirculationPerSecond(), heart.getCirculationPerSecond(), maximumBlood);
    float oxygenUse =
        valueOrDefault(
            heart.hasOxygenUsePerSecond(), heart.getOxygenUsePerSecond(), DEFAULT_HEART_OXYGEN_USE);
  ...
  private void updateLiver(PhysiologyState state) {
    ...
    float oxygenUse =
        valueOrDefault(
            liver.hasOxygenUsePerSecond(), liver.getOxygenUsePerSecond(), DEFAULT_LIVER_OXYGEN_USE);
    float toxinRecovery =
        positive(liver.getDetoxification()) * organFunction * LIVER_TOXIN_RECOVERY_PER_DETOX_UNIT;
  ...
  private void updateBrain(PhysiologyState state) {
    ...
    float oxygenUse =
        valueOrDefault(
            brain.hasOxygenUsePerSecond(), brain.getOxygenUsePerSecond(), DEFAULT_BRAIN_OXYGEN_USE);
  ...
  private Entity applyChanges(Entity entity, PhysiologyState state) {
    ...
    for (Map.Entry<String, Float> chemical : state.bloodChemicals.entrySet()) {
      if (chemical.getValue() > 0) {
        updatedCardiovascularSystem.addChemicals(
            Chemical.newBuilder().setName(chemical.getKey()).setAmount(chemical.getValue()));
      }
    }
    ...
    updatedDamage.setToxinDamage(
        Math.max(0, positive(updatedDamage.getToxinDamage()) - state.toxinRecovery));

    return entity.toBuilder().setOrgans(updatedOrgans).setDamage(updatedDamage).build();
  }

  private static void updateStomachEntity(
      Entity updatedBody, OrganSlots organSlots, EntityManager entityManager) {
    ...
    Entity updatedStomachEntity =
        stomachEntity.toBuilder()
            .setOrganComponent(
                stomachEntity.getOrganComponent().toBuilder()
  ...
  private static void updateHeartEntity(
    ...
    Entity updatedHeartEntity =
        heartEntity.toBuilder()
            .setOrganComponent(
                heartEntity.getOrganComponent().toBuilder()
  ...
  private static void loadHeartFromEntity(
      Organs.Builder installedOrgans, EntityManager entityManager, int organEntityId) {
