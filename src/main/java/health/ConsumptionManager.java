...
public class ConsumptionManager {
  ...
  public void consume(Entity foodItem, Entity consumingEntity) {
    ...
    for (int i = 0; i < foodItem.getChemicalsCount(); i++) {
      ...
      float acceptedUnits =
          Math.min(requestedUnits, chemicalManager.getRemainingStomachCapacity(consumingEntity));
      if (acceptedUnits <= 0) continue;

      consumingEntity =
          chemicalManager.addToStomach(consumingEntity, chemical.getName(), acceptedUnits);
      Chemical newChemical =
          chemical.toBuilder().setAmount(Math.max(0, chemical.getAmount() - acceptedUnits)).build();
