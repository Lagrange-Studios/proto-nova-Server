...
public final class ChemicalManager {
  ...
  public Entity addToStomach(Entity entity, String chemicalName, float requestedUnits) {
    ...
    Stomach.Builder updatedStomach =
        stomach.toBuilder().clearChemicals().clearContents().setChemicalCapacity(capacity);
  ...
  public Entity injectIntoCirculation(Entity entity, String chemicalName, float requestedUnits) {
    ...
    CardiovascularSystem cardiovascular =
        organs.hasCardiovascularSystem()
    ...
    float currentBlood = heart == null ? 0 : clamp(heart.getBlood(), 0, maximumBlood);
    float capacity =
    ...
    CardiovascularSystem.Builder updatedCardiovascular =
        cardiovascular.toBuilder().clearChemicals().setFluidCapacity(capacity);
  ...
  private Entity storeStomach(Entity body, Stomach stomach) {
    ...
    if (organEntity != null
        && organEntity.hasOrganComponent()
        && organEntity.getOrganComponent().hasStomach()) {
      OrganComponent updatedComponent =
          organEntity.getOrganComponent().toBuilder().setStomach(stomach).build();
      entityManager.updateEntity(
          organEntity.toBuilder().setOrganComponent(updatedComponent).build());
      return body;
    }

    Organs updatedOrgans = body.getOrgans().toBuilder().setStomach(stomach).build();
