...
public class ChemicalDigestionManager {
  ...
  public Entity processEntityChemicals(Entity entity) {
    if (entity == null || !entity.hasOrgans() || !entity.getOrgans().hasCardiovascularSystem()) {
      return entity;
    }
    ...
    for (Chemical chemical : updatedCirculation.getChemicalsList()) {
      ...
      if (isInTemperatureRange(updatedEntity, definition)) {
        ...
        updatedCirculation =
            applyNutritionEffect(updatedCirculation, getSaturation(definition, overdose));
      }

      float remainingAmount = Math.max(0, amount - positive(definition.getUnitsPerSecond()));
      if (remainingAmount > 0) {
        chemicalChanges.addChemicals(chemical.toBuilder().setAmount(remainingAmount));
      }
    }

    updatedCirculation =
        updatedCirculation.toBuilder()
            .clearChemicals()
  ...
  private void applyDamageEffects(
      Entity.Builder entity, ChemicalDefinition definition, boolean overdose) {
    ...
    Damage.Builder updated =
        current.toBuilder()
            .setBruteDamage(
                resolveDamage(
                    current.getBruteDamage(), damage.getBruteDamage(), healing.getBruteDamage()))
            .setAsphyxiationDamage(
                resolveDamage(
                    current.getAsphyxiationDamage(),
                    damage.getAsphyxiationDamage(),
                    healing.getAsphyxiationDamage()))
            .setBurnDamage(
                resolveDamage(
            ...
            .setGeneticDamage(
                resolveDamage(
                    current.getGeneticDamage(),
                    damage.getGeneticDamage(),
                    healing.getGeneticDamage()))
            .setStructuralDamage(
                resolveDamage(
                    current.getStructuralDamage(),
                    damage.getStructuralDamage(),
                    healing.getStructuralDamage()))
            .setBleedingPerSecond(
                resolveDamage(
                    current.getBleedingPerSecond(),
                    damage.getBleedingPerSecond(),
                    healing.getBleedingPerSecond()));
  ...
  private static CardiovascularSystem applyNutritionEffect(
      CardiovascularSystem cardiovascularSystem, Double saturation) {
  ...
  private static boolean isInTemperatureRange(
      Entity.Builder entity, ChemicalDefinition definition) {
