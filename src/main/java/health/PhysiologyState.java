...
final class PhysiologyState {
  ...
  PhysiologyState(Entity entity, float breathableOxygen) {
    ...
    oxygen = clamp(cardiovascularSystem.getOxygen(), 0, oxygenCapacity);

    if (cardiovascularSystem.hasMaxElectricalPower()) {
      maxPower = positive(cardiovascularSystem.getMaxElectricalPower());
    } else {
      maxPower = positive(cardiovascularSystem.getElectricalPower());
    }

    power = clamp(cardiovascularSystem.getElectricalPower(), 0, maxPower);

    maxNutrition = OrganEnergy.maximumNutrition(cardiovascularSystem);

    nutrition = clamp(cardiovascularSystem.getNutrition(), 0, maxNutrition);
    ...
    if (cardiovascularSystem.hasFluidCapacity()) {
      requestedFluidCapacity = positive(cardiovascularSystem.getFluidCapacity());
    } else {
    ...
    for (Chemical chemical : cardiovascularSystem.getChemicalsList()) {
      addChemical(bloodChemicals, chemical.getName(), chemical.getAmount());
    }
    ...
    if (organs.hasStomach()) {
      ...
      for (Chemical chemical : stomach.getContentsList()) {
        addChemical(stomachChemicals, chemical.getName(), chemical.getAmount());
      }
  ...
  void useEnergy() {
    ...
    if (organs.hasHeart()) {
      ...
      totalNutritionUse +=
          OrganEnergy.nutritionUsePerSecond(
  ...
  float getOrganFunction(OrganStatus organStatus) {
    float organHealth = organCondition(organStatus.hasHealth(), organStatus.getHealth());
    float organEfficiency =
        organCondition(organStatus.hasEfficiency(), organStatus.getEfficiency());
  ...
  void digest(float absorptionAmount) {
    ...
    float availableChemicalSpace = chemicalSpace - currentBloodstreamChemicalAmount;
    availableChemicalSpace = Math.max(0, availableChemicalSpace);

    if (totalStomachChemicalAmount <= 0 || absorptionAmount <= 0 || availableChemicalSpace <= 0) {
      return;
    }
    ...
    for (Map.Entry<String, Float> stomachChemical : stomachChemicals.entrySet()) {
      ...
      addChemical(bloodChemicals, stomachChemical.getKey(), amountMovedToBloodstream);
    }
  }
  ...
  private static void addChemical(
      Map<String, Float> chemicals, String chemicalName, float chemicalAmount) {
  ...
  private static void limitChemicals(Map<String, Float> chemicals, float chemicalCapacity) {
