...
public class HealthManager {
  ...
  public HealthManager(
      EntityManager entityManager, Console console, LootTableManager lootTableManager) {
  ...
  public Entity prepareEntityState(Entity entity) {
    ...
    if (!usesHealthSystem(entity)) {
      return entity.toBuilder().setMaxSpeed(maximumSpeed).build();
    }
  ...
  public boolean checkCrit(Entity entity) {
    return usesHealthSystem(entity) && entity.getCritHealth() <= Health.getDamage(entity);
  }
  ...
  public boolean checkDeath(Entity entity) {
    return usesHealthSystem(entity) && entity.getMaxHealth() <= Health.getDamage(entity);
  }
  ...
  private static double calculateHealthSpeedMultiplier(
      Entity entity, double damage, boolean alive) {
    if (!alive || damage >= entity.getCritHealth()) return 0;

    double criticalThreshold = entity.getCritHealth();
    if (damage >= criticalThreshold * TraumaState.MORTALLY_WOUNDED.getTraumaPercentAsDecimal())
      return 0.3;
    if (damage >= criticalThreshold * TraumaState.SEVERELY_INJURED.getTraumaPercentAsDecimal())
      return 0.6;
    if (damage >= criticalThreshold * TraumaState.INJURED.getTraumaPercentAsDecimal()) return 0.75;
    if (damage >= criticalThreshold * TraumaState.MINOR_INJURIES.getTraumaPercentAsDecimal())
      return 0.9;
    return 1;
  }
  ...
  private void gibEntity(Entity entity) {
    ...
    dropOrgans(entity);
    Entity bodyWithoutInstalledOrgans = entity.toBuilder().clearOrganSlots().build();
