...
public class PathfindingHandler {
  ...
  public void newAgent(int id, Vector position, String parameter) {
    if (!agents.contains(id)) {
      Agent agent =
          new Agent(
              id, entityManager, entityFinder, server, combatManager, getParameter(parameter));
  ...
  public void removeEntity(Entity entity) {
    if (agents.contains(entity.getId())) agents.remove(entity.getId());
  }
  ...
  private void loadParameters() {
    try (ScanResult scanResult = new ClassGraph().acceptPackages("agentParameters").scan()) {
      for (ClassInfo classInfo : scanResult.getAllClasses()) {
        ...
        try {
          AgentParameter newParameter =
              (AgentParameter) staticClass.getDeclaredConstructor().newInstance();
