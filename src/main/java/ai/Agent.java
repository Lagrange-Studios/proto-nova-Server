...
public class Agent {
  ...
  public void tick(boolean canFindGoal) {
    ...
    if (entity != null
        && (!parameters.mustBeAlive() || parameters.mustBeAlive() && entity.getAlive())) {
      ArrayList<Entity> entities =
          canFindGoal
              ? entityFinder.getAllEntitiesInRadis(entity, parameters.getRange())
              : entityFinder.getAllEntitiesInRadis(entity, minimumSearch);
      ...
      if (goal != null) {
        ...
        entity = entity.toBuilder().setPosition(difference).build();
        ...
        for (Entity closeEntity : entities) {
          if (EntityCollision.checkCollision(closeEntity, entity)) {
            ...
            if (closeEntity.getCanCollide()) revertPosition = true;
          }
        }

        if (revertPosition) entity = entity.toBuilder().setPosition(oldPosition).build();

        entityManager.updateEntity(entity);
        double distanceSqaured = VectorMath.distanceSquared(difference, goal);

        // complete path
        if (distanceSqaured <= minimumDistanceSquared && parameters.closeOnPathEnd())
          completed = true;
  ...
  public void findGoal(ArrayList<Entity> closeEntities) {
    ...
    if (closestTarget != null) setGoal(closestTarget.getId());
