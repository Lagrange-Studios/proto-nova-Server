...
public class EntityFinder {
  ...
  public ArrayList<Entity> getAllEntitiesInRadius(Vector start, int mapId, double radius) {
    ...
    if (Math.pow(radiusInChunks, 2) < 10) {
      ...
      for (int i = 0; i < chunkList.size(); i += CHUNKS_PER_THREAD) {
        Future<ArrayList<Entity>> future =
            server.threadPool.submit(
                findEntitiesInChunkThread(
                    chunkList,
                    i,
                    Math.min(i + CHUNKS_PER_THREAD, chunkList.size()),
                    start,
                    radiusSquared));
  ...
  private Callable<ArrayList<Entity>> findEntitiesInChunkThread(
      ArrayList<Chunk> list, int indexStart, int indexEnd, Vector start, double radiusSquared) {
          ...
          for (int i = indexStart; i < indexEnd; i++) {
            ...
            for (int id : chunk.getEntityIdsList()) {
              ...
              double distanceSquared =
                  VectorMath.distanceSquared(start, selectedEntity.getPosition());
  ...
  private ArrayList<Entity> findEntitiesInChunk(
      ArrayList<Chunk> list, Vector start, double radiusSquared) {
    // split each group chunk search into threads for speed
