...
public class ChunkManager {
    ...
    return chunks.computeIfAbsent(
        ...
        selectedMapId -> {
          if (!planeManager.planeExists(selectedMapId) && selectedMapId != 0) {
            System.err.println(
                "Warning: just tried to make chunks for plane id: "
                    + selectedMapId
  ...
  private void addEntityToChunk(
      ConcurrentHashMap<Coordinate, Chunk> chunkMap, Coordinate coordinate, Entity entity) {
    chunkMap.compute(
        coordinate,
        (selectedCoordinate, selectedChunk) -> {
          Chunk.Builder chunkBuilder =
              selectedChunk == null
  ...
  private void addSoundToChunk(
      ConcurrentHashMap<Coordinate, Chunk> chunkMap, Coordinate coordinate, Audio audio) {
    chunkMap.compute(
        coordinate,
        (selectedCoordinate, selectedChunk) -> {
          Chunk.Builder chunkBuilder =
  ...
  private void addChatMessageToChunk(
      ConcurrentHashMap<Coordinate, Chunk> chunkMap, Coordinate coordinate, ChatMessage message) {
    chunkMap.compute(
        coordinate,
        (selectedCoordinate, selectedChunk) -> {
          Chunk.Builder chunkBuilder =
  ...
  public void addEntity(Entity entity) {
    ...
    addEntityToChunk(
        getPlaneChunks(entity),
        CoordinateConverter.toChunkCoordinates(entity.getPosition()),
        entity);
  }
  ...
  public boolean addSound(Audio audio) {
    ...
    addSoundToChunk(
        getPlaneChunks(audio.getMap()), CoordinateConverter.toChunkCoordinates(position), audio);
    return true;
  }
  ...
  public void addChatMessage(ChatMessage message) {
    ...
    addChatMessageToChunk(
        getPlaneChunks(message.getMap()),
        CoordinateConverter.toChunkCoordinates(position),
        message);
  }

  public void removeAllChatMessages() {
    for (ConcurrentHashMap<Coordinate, Chunk> planeChunks : chunks.values()) {
      planeChunks.replaceAll(
          (coordinate, chunk) ->
              chunk.getChatsCount() == 0 ? chunk : chunk.toBuilder().clearChats().build());
    }
  }
  ...
  public void removeAllSounds() {
    for (ConcurrentHashMap<Coordinate, Chunk> planeChunks : chunks.values()) {
      planeChunks.replaceAll(
          (coordinate, chunk) ->
              chunk.getSoundsCount() == 0 ? chunk : chunk.toBuilder().clearSounds().build());
    }
  }

  private void removeEntityFromChunk(
      ConcurrentHashMap<Coordinate, Chunk> chunkMap, Coordinate coordinate, Entity entity) {
    chunkMap.computeIfPresent(
        coordinate,
        (selectedCoordinate, selectedChunk) -> {
  ...
  public void updateEntityChunck(Entity oldEntity, Entity newEntity) {
    ...
    if (canUseWorldChunks(newEntity)) {
      addEntityToChunk(
          getPlaneChunks(newEntity),
          CoordinateConverter.toChunkCoordinates(newEntity.getPosition()),
          newEntity);
    }
  }
  ...
  static boolean isWorldEntity(Entity entity) {
    ...
    return !entity.hasOrganComponent() || !entity.getOrganComponent().hasInstalledInEntityId();
  }
}
