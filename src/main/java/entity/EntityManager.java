...
public class EntityManager {
  ...
  public EntityManager(
      ServerLoader serverLoader, Console console, ArrayList<Player> playerList, Server server) {
  ...
  /**
   * Updates the entity list with the new value and checks for movement both position wise and map
   * change then updates the chunk manager
   *
   * @param entity
   */
  public synchronized void updateEntity(Entity entity) {
    boolean isFirstInsertion =
        !entities.containsKey(entity.getId()) || reservedEntityIds.remove(entity.getId());
    boolean installedOrgan =
        entity.hasOrganComponent() && entity.getOrganComponent().hasInstalledInEntityId();

    if (entity.getMaxSpeed() <= 0) {
      entity = entity.toBuilder().setMaxSpeed(7.5).build();
    }
    ...
    if (!isFirstInsertion && !installedOrgan) {
      ...
      if (!oldEntity.getPosition().equals(entity.getPosition())
          || oldEntity.getMap() != entity.getMap()) {
    ...
    for (int id : entity.getInventorySlotsMap().values()) {
      Entity item = getEntity(id).toBuilder().setPosition(entity.getPosition()).build();
  ...
  /**
   * Decrements the amount of a entity Also force updates the entity with the new amount
   *
  ...
  public Entity decrementAmount(Entity entity) {
    ...
    if (entity.getStackable()) {
      entity = entity.toBuilder().setAmount(entity.getAmount() - 1).build();
  ...
  public Entity decrementSlot(Entity entity, String slot) {
    ...
    if (item != null) {
      ...
      if (!item.getStackable() || item.getAmount() == 0) {
        entity = entity.toBuilder().removeInventorySlots(slot).build();
      }
  ...
  public synchronized void removeEntity(Entity entity) {
    ...
    if (entity.hasOrganSlots()) {
      removeInstalledOrgan(
          entity.getOrganSlots().hasHeartEntityId()
              ? entity.getOrganSlots().getHeartEntityId()
              : -1);
      removeInstalledOrgan(
          entity.getOrganSlots().hasLungsEntityId()
              ? entity.getOrganSlots().getLungsEntityId()
              : -1);
      removeInstalledOrgan(
          entity.getOrganSlots().hasLiverEntityId()
              ? entity.getOrganSlots().getLiverEntityId()
              : -1);
      removeInstalledOrgan(
          entity.getOrganSlots().hasBrainEntityId()
              ? entity.getOrganSlots().getBrainEntityId()
              : -1);
      removeInstalledOrgan(
          entity.getOrganSlots().hasStomachEntityId()
              ? entity.getOrganSlots().getStomachEntityId()
              : -1);
    }
  ...
  /**
   * Installs health validation and normalizes entities loaded from an existing world before the
  ...
  public synchronized void setHealthManager(HealthManager healthManager) {
    ...
    entities.replaceAll(
        (id, entity) ->
            healthManager.prepareEntityState(
                entity.toBuilder()
  ...
  public Entity simulateVelocity(Entity entity, int tps, float speedMultiplier) {
    ...
    Entity entityXAxis =
        checkCollision(
            EntitySimulation.simulateVelocityXAxis(
                entity, tps, speedMultiplier, server.updateMultiplier),
            entity,
            closeEntities);
    ...
    if (entityXAxis.getPosition().equals(entity.getPosition())) {
      ...
      Vector newVeloicty = entity.getVelocity().toBuilder().setX(0).build();

      entity = entity.toBuilder().setVelocity(newVeloicty).build();
    } else entity = entityXAxis;

    Entity entityYAxis =
        checkCollision(
            EntitySimulation.simulateVelocityYAxis(
                entity, tps, speedMultiplier, server.updateMultiplier),
            entity,
            closeEntities);
    ...
    if (entityYAxis.getPosition().equals(entity.getPosition())) {
      ...
      Vector newVeloicty = entity.getVelocity().toBuilder().setY(0).build();
  ...
  private Entity checkCollision(
      Entity updatedEntity, Entity originalEntity, ArrayList<Entity> closeEntities) {
  ...
  public void recalculateEntity(Entity entity) {
    ...
    Entity recalculated = healthManager == null ? stored : healthManager.prepareEntityState(stored);
