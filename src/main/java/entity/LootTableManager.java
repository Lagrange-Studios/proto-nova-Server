...
public class LootTableManager {
  ...
  private ArrayList<Entity> dropLootEntry(Entity sourceEntity, lootTableItem lootEntry) {
    ...
    Entity firstDroppedEntity =
        assetManager.getEntity(lootEntry.getItemName(), sourceEntity.getMap());
    ...
    if (firstDroppedEntity.getStackable()) {
      firstDroppedEntity = firstDroppedEntity.toBuilder().setAmount(requestedAmount).build();
    ...
    for (int droppedAmount = 1; droppedAmount < requestedAmount; droppedAmount++) {
      Entity additionalDroppedEntity =
          assetManager.getEntity(lootEntry.getItemName(), sourceEntity.getMap());
