...
public class AssetManager {
  ...
  public AssetManager(
      ...
      HashMap<String, Entity> entityAssets,
      Console console,
  ...
  public Entity getEntity(String name, int mapId, Vector newPosition) {
    ...
    if (entityAssets.containsKey(name)) {
      Entity entity =
          entityAssets.get(name).toBuilder()
  ...
  private void setOrganSlot(OrganSlots.Builder slots, String assetName, Entity body, int slot) {
    ...
    OrganComponent component =
        asset.getOrganComponent().toBuilder().setInstalledInEntityId(body.getId()).build();
    Entity organ =
        asset.toBuilder()
            .setId(entityManager.reserveNewEntityId())
            .setMap(-1)
    ...
    switch (slot) {
      case 0:
        slots.setHeartEntityId(organ.getId());
        break;
      case 1:
        slots.setLungsEntityId(organ.getId());
        break;
      case 2:
        slots.setLiverEntityId(organ.getId());
        break;
      case 3:
        slots.setBrainEntityId(organ.getId());
        break;
      case 4:
        slots.setStomachEntityId(organ.getId());
        break;
      default:
        break;
    }
