...
public class CraftingManager {
  ...
  public CraftingManager(
      ...
      ArrayList<CraftingRecipe> loadedRecipes,
      Console console,
  ...
  public CraftingRecipe getRecipe(String item1Name, String item2Name) {
    if (itemsToRecipes.containsKey(item1Name + item2Name))
      return itemsToRecipes.get(item1Name + item2Name);
    else if (itemsToRecipes.containsKey(item2Name + item1Name))
      return itemsToRecipes.get(item2Name + item1Name);
    return null;
  }
  ...
  public Entity attemptCraftingRecipe(Entity craftingEntity, Entity component) {
    ...
    if (heldComponent != null && component != null) {
      ...
      if (recipe != null
          && (!recipe.hasTileResult()
              || planeManager.getTileAt(component).getSurfaceTexture().equals(""))
  ...
  private boolean checkComponent(Entity component, CraftingComponent craftingComponent) {
    ...
    for (String key : craftingComponent.getMinimumDataIntValueMap().keySet()) {
      ...
      if (!component.containsCustomData(key)
          || component.getCustomDataMap().get(key).getIntValue()
              < craftingComponent.getMinimumDataIntValueMap().get(key)) return false;
    }
  ...
  private void attemptToAddRecipe(String key, CraftingRecipe value) {
    ...
    if (itemsToRecipes.containsKey(key)) {
      String warningMessage =
          "Warning: Could not add recipe for "
              + value.getResult()
              + " becuase it conflicts with the recipe for "
              + itemsToRecipes.get(key).getResult();
      console.print(warningMessage);
      System.err.print(warningMessage);
    } else if (!assetManager.containsEntity(value.getItem1().getName()))
      console.print(
          "Warning: Could not add recipe for "
              + value.getResult()
              + " becuase the asset manager is missing: "
              + value.getItem1().getName());
    else if (!assetManager.containsEntity(value.getItem2().getName()))
      console.print(
          "Warning: Could not add recipe for "
              + value.getResult()
              + " becuase the asset manager is missing: "
              + value.getItem2().getName());
    else if (!value.getTileResult() && !assetManager.containsEntity(value.getResult()))
      console.print(
          "Warning: Could not add recipe for "
              + value.getResult()
              + " becuase the asset manager is missing: "
