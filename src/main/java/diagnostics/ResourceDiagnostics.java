...
public class ResourceDiagnostics {
  ...
  private static final ConcurrentHashMap<String, AtomicInteger> THREAD_NAME_SEQUENCES =
  ...
  public synchronized void beginSession() {
    ...
    if (threadBean instanceof com.sun.management.ThreadMXBean) {
      ...
      if (sunThreadBean.isThreadAllocatedMemorySupported()
          && !sunThreadBean.isThreadAllocatedMemoryEnabled()) {
        try {
  ...
  private static String uniqueThreadName(String prefix) {
    int sequence =
        THREAD_NAME_SEQUENCES
            .computeIfAbsent(prefix, ignored -> new AtomicInteger())
            .incrementAndGet();
  ...
  public void recordEntityCpu(int entityId, long nanos) {
    ...
    if (counters != null && nanos > 0)
      counters.computeIfAbsent(entityId, id -> new LongAdder()).add(nanos);
  }

  public void recordEntityNetwork(int entityId, long bytes) {
    ConcurrentHashMap<Integer, LongAdder> counters = entityNetworkBytes;
    if (counters != null && bytes > 0)
      counters.computeIfAbsent(entityId, id -> new LongAdder()).add(bytes);
  }
  ...
  public synchronized List<ThreadSnapshot> getThreadSnapshots(Set<Long> stackThreadIds) {
    ...
    for (int index = 0; index < ids.length; index++) {
      ...
      double allocationRate =
          allocated < 0
              ? -1
              : Math.max(0, (allocated - priorAllocated) * 1_000_000_000.0 / elapsed);
      ...
      if (stackThreadIds != null && stackThreadIds.contains(id)) {
        ...
        stack =
            stackInfo == null
                ? "Thread ended before its stack could be captured."
                : formatStack(stackInfo);
      }
      result.add(
          new ThreadSnapshot(
              id,
              ...
              value(txCounters == null ? null : txCounters.get(id)),
              stack));
    }
  ...
  public synchronized List<EntitySnapshot> findEntities(String query, int maximumResults) {
    ...
    try {
      if (!normalized.isEmpty()) requestedId = Integer.valueOf(normalized);
    ...
    result.sort(
        Comparator.comparing(EntitySnapshot::getName, String.CASE_INSENSITIVE_ORDER)
  ...
  private static long value(LongAdder value) {
  ...
  private static String formatStack(ThreadInfo info) {
    ...
    return builder.length() == 0
        ? "No Java stack available (native or idle thread)."
        : builder.toString();
  }
  ...
  public static final class ProcessSnapshot {
    ...
    ProcessSnapshot(
      ...
      this.networkWritten = networkWritten;
    }
  }
  ...
  public static final class ThreadSnapshot {
    ...
    ThreadSnapshot(
      ...
      this.allocationBytesPerSecond = allocationBytesPerSecond;
  ...
  public static final class EntitySnapshot {
    ...
    EntitySnapshot(
        Entity entity, double cpuPercent, long cpuNanos, int serializedBytes, long networkBytes) {
