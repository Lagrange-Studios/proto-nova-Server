...
public class EnviromentGenerator {
  ...
  public void generateEnviroment(Plane plane, String type) {
    ...
    for (int i = 0; i < enviromentDataFiles.length; i++) {
      ...
      JSONObject generationData =
          new JSONObject(FileReader.readJSONFile(enviromentDataFiles[i].getPath()));

      if (!assetManager.containsEntity(generationData.getString("name")))
        continue; // Check if we even have the entity
      ...
      for (int u = 0;
          u < Math.floor(validTiles.size() / generationData.getDouble("tileDensity"));
          u++) {
        ...
        Vector newPosition =
            CoordinateConverter.toVector(validTiles.get(randomIndex).getCoordinate());

        newPosition =
            newPosition.toBuilder()
