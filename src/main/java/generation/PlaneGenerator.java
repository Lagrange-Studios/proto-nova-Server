...
public class PlaneGenerator {
  ...
  public Plane generatePlane(int sizeX, int sizeY, String generationType) {
    ...
    Coordinate size = Coordinate.newBuilder().setX(sizeX).setY(sizeY).build();

    Builder plane = Plane.newBuilder().setId(planeId).setSize(size);

    long seed = System.currentTimeMillis();

    JSONObject generationValues =
        new JSONObject(
            FileReader.readJSONFile("assets/generation/terrain/" + generationType + ".json"));
      ...
      for (int y = -sizeY / 2; y <= sizeY / 2; y++) {
        ...
        double value =
            OpenSimplex2S.noise2(
                seed,
                x * generationValues.getDouble("frequency"),
                y * generationValues.getDouble("frequency"));
        String name = "null";
        ...
        Coordinate coordinate = Coordinate.newBuilder().setX(x).setY(y).build();
        ...
        Tile tile =
            Tile.newBuilder()
                ...
                .setMovementDifficulty(
                    movementDifficulties == null ? 1.0f : movementDifficulties.optFloat(name, 1.0f))
                .build();
