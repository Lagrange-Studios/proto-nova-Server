...
public class ServerSocketHandler {
        ...
        ResourceDiagnostics.newThread(
            ...
            () -> {
              try {
                ...
                console.print(
                    "Server ready: secure game listener active on TCP port "
                        + PORT
                ...
                while (!closing && !serverSocket.isClosed()) {
                  ...
                  try {
                    ...
                    Player player =
                        new Player(clientSocket, console, packetReciver, this, gameAuthService);
  ...
  public boolean addPlayerToGame(Player player) {
    synchronized (this) {
      if (closing
          || player.getState() == enums.Player.State.DISCONNECTED
          || playerList.contains(player)) return false;
  ...
  public void removePlayer(Player player) {
    ...
    console.print(
        "Removed player: " + (player.getUsername() != null ? player.getUsername() : "unknown"));
    if (player.data != null) {
