...
public class PacketReciver {
  ...
  private final double reconcileCoefficient =
      20; // Multiplier on one tick of movement; higher = more tolerant of client drift before
  // reconciling
  ...
  private void recivePacketLocked(Player player, ClientToServerPacket packet) {
    ...
    if (healthManager.canPerformActions(serverEntity)) {
      for (Action action : packet.getActionsList()) {
        ...
        if (action.getActionType() != ActionType.Interact) {
          ...
          // so we need the velocity changes persisted first to prevent the re-fetch from getting
          // stale data.
      ...
      Vector clearedVelocity = serverEntity.getVelocity().toBuilder().setX(0).setY(0).build();
      serverEntity = serverEntity.toBuilder().setVelocity(clearedVelocity).build();
    }
    ...
    double reconcileDistance =
        Math.max(
            0.05, (serverEntity.getSpeed() * speedMultiplier / server.TPS) * reconcileCoefficient);
  ...
  private Entity applyCharacterAppearance(Player player, Entity serverEntity, Entity clientEntity) {
    ...
    for (String tag : clientEntity.getTagsList()) {
      ...
      if (update != null || !CharacterAppearanceCodec.isValidUpdate(tag)) {
        console.print(
            "WARNING: Rejected invalid character appearance from " + player.getUsername());
        return serverEntity;
      }
      update = tag;
    }
