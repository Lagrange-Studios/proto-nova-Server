...
public class PacketMaker {
  ...
  public PacketMaker(
      ...
      SoundFinder soundFinder,
      ChatFinder chatFinder,
      ...
      case AWAITING_SERVER_PACKET:
        ...
        for (int i = 0; i < players.size(); i++) {
          if (players.get(i) != player
              && players.get(i).getUsername().equals(player.getUsername())) {
            player.disconnect();
            break;
        ...
        if (player.getState() != State.DISCONNECTED) {
          if (player.data == null) {
            ...
            if (player.data.getEntityId() == 0
              ...
              player.data = player.data.toBuilder().setEntityId(newEntity.getId()).build();
            }
            if (serverSaver != null) {
  ...
  private void sendNormalPacket(Player player) {
    ...
    Builder packet = ServerToClientPacket.newBuilder().setMindId(player.data.getEntityId());
    ...
    for (int x = -TILE_RENDER_X; x <= TILE_RENDER_X; x++) {
      for (int y = -TILE_RENDER_Y; y <= TILE_RENDER_Y; y++) {
        ...
        if (currentPlane == null)
          System.out.println("ERROR IMPORTANT PRINTING MAP ID: " + playerEntity.getMap());
    ...
    ArrayList<Entity> foundEntities =
        entityFinder.getAllEntitiesInRadis(playerEntity, renderDistance);
    ...
    for (Entity entity : foundEntities) {
      ...
      if (entity.getName().equals("human"))
        updateInventory(
            entity, packet, deleteList, updateList, entitiesSent, entitiesSentThisPacket);
    }
    ...
    for (int id : entitiesSent.toArray(new Integer[0])) {
      if (deleteList.contains(id)
        ...
        // diagnostics.recordEntityNetwork(updatedEntity.getId(),
        // updatedEntity.getSerializedSize());
    ...
    packet.setCurrentCelestialObject(
        celestialObjectManager.getCelestialObjectFromPlane(currentPlane));
  ...
  private void updateInventory(
      ...
      HashSet<Integer> updateList,
      HashSet<Integer> entitiesSent,
      HashSet<Integer> entitiesSentThisPacket) {
    for (int id : entity.getInventorySlotsMap().values()) {
      Entity inventoryItem = entityManager.getEntity(id);
      if (inventoryItem != null
          && (!entitiesSent.contains(inventoryItem.getId()) || updateList.contains(id))) {
        packet.addEntities(inventoryItem);
        // diagnostics.recordEntityNetwork(inventoryItem.getId(),
        // inventoryItem.getSerializedSize());
