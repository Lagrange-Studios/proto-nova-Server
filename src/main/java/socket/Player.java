...
public class Player {
  ...
  private static final long LEVEL_3_RECHECK_MILLIS =
      Math.multiplyExact(main.ServerConfig.getInstance().getSecurityLevel3RecheckSeconds(), 1_000L);
  private static final long LEVEL_3_GRACE_MILLIS =
      Math.multiplyExact(
          main.ServerConfig.getInstance().getSecurityLevel3ApiGraceSeconds(), 1_000L);
  private static final int IDLE_TIMEOUT_MILLIS =
      Math.multiplyExact(main.ServerConfig.getInstance().getGameSocketIdleTimeoutSeconds(), 1_000);
  ...
  private final ArrayBlockingQueue<byte[]> outboundPackets =
  ...
  long lastPlayerDataSent = 0;

  public Player(
      Socket socket,
      Console console,
      PacketReciver packetReciver,
    ...
    try {
      ...
      while (state != State.DISCONNECTED) {
        ...
        if (username == null) {
          ...
          if (SECURITY_LEVEL >= 2) {
            JoinResult result =
                gameAuthService.consumeJoinTicket(
          ...
          if (serverSocketHandler != null) {
            if (!serverSocketHandler.addPlayerToGame(this)) {
              console.print(
                  "WARNING: Rejected a duplicate or unavailable player session: " + username);
              disconnect();
              return;
            }
            addedToGame = true;
          ...
          if (!allowGameplayPacket()) {
            console.print(
                "WARNING: Disconnected a client sending packets too quickly: " + username);
            disconnect();
            return;
          }
  ...
  private void sendSecurityHandshake() throws IOException {
    ClientDistribution distribution =
        main.ServerConfig.getInstance().isStatusHttpEnabled()
            ? ClientDistribution.getIfAvailable()
            : null;
    ...
    setOptionalIntField(
        handshake, "setServerTicksPerSecond", main.ServerConfig.getInstance().getTicksPerSecond());
  ...
  private static void setOptionalIntField(Object target, String methodName, int value)
      throws IOException {
    try {
    ...
    if (target instanceof ServerHandshake.Builder) {
      ...
      UnknownFieldSet.Builder unknownFields =
          UnknownFieldSet.newBuilder(((ServerHandshake.Builder) target).getUnknownFields());
  ...
  private boolean allowApiSession() {
    ...
    if (apiUnavailableSince != 0 && now - apiUnavailableSince >= LEVEL_3_GRACE_MILLIS) {
      console.print(
          "Disconnected a player because the security API grace period expired: " + username);
      disconnect();
      return false;
    }
  ...
  public void send(byte[] bytes) {
    ...
    if (!outboundPackets.offer(bytes)) {
      console.print(
          "Disconnected a client that could not receive data fast enough: "
