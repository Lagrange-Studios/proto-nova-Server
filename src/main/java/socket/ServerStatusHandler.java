...
public class ServerStatusHandler {
  ...
  public void start() throws IOException {
    try {
      ...
      HttpsServer httpsServer =
          HttpsServer.create(new InetSocketAddress(bindAddress, HTTP_PORT), 16);
      ...
      httpServer.start();
      console.print(
          "Status listener active on https://" + bindAddress + ":" + HTTP_PORT + "/status");
    } catch (IOException e) {
      throw e;
    } catch (Exception e) {
  ...
  private final class ClientManifestHandler extends DownloadHandler {
    @Override
    void send(HttpExchange exchange, ClientDistribution distribution) throws IOException {
